...
namespace SistemaGestionData
{
    public static class ProductoData
    {
                        ...
                        using (SqlDataReader dr = comando.ExecuteReader())
                        {
                            if (dr.HasRows)
                            {
                                while (dr.Read())
                                {
                                    ...
                                    producto.Costo = Convert.ToDecimal(dr["PrecioVenta"]);
        ...
        {
            string connectionString = @"Server=Julen\SQLEXPRESS01;DataBase=SistemaGestion;Trusted_Connection=True";
            string query = "UPDATE Producto " + "SET Descripciones = @Descripcion ,Costo = @Costo, PrecioVenta = @PrecioVenta,Stock = @Stock, IdUsuario=@IdUsuario " +
                " WHERE Id = @Id";
            try
            {
                using (SqlConnection conexion = new SqlConnection(connectionString))
